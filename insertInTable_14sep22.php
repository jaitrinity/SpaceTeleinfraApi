...
if($insertType == "employeeLocationMapping" && $methodType === 'POST'){
	...
	if($actionType == 'insert'){
		// $locationSql = "";
		// if($isOR == false){
		// 	$locationSql = "SELECT * FROM `Location` WHERE `State` in ('".implode("','", explode(",", $state))."') and `Tenent_Id` = '$tenentId' ";
		// }
		// else{
		// 	$locationSql = "SELECT * FROM `Location` WHERE `Site_Id` in ('".implode("','", explode(",", $siteId))."') 
		// 	and `Tenent_Id` = '$tenentId' ";
		// }

		// // echo $locationSql;

		// $query = mysqli_query($conn,$locationSql);
		...
		// 	$insertTable = "INSERT INTO `EmployeeLocationMapping`(`State`, `Name`, `LocationId`, `Role`, `Emp_Id`, `Tenent_Id`)";
			
		// 	$insertData = "('$state', '$rowLocName', '$rowLocationId', '$role', '$employee', '$tenentId')";

		// 	$insertEmpLocMapping = $insertTable." values ".$insertData;
		// 	mysqli_query($conn,$insertEmpLocMapping);
		// }

		// $insertEmpLocMapping = "INSERT INTO `EmployeeLocationMapping`(`State`, `Name`, `LocationId`, `Role`, `Emp_Id`, `Tenent_Id`) ";
		// if($isOR == false){
		// 	$insertEmpLocMapping .= "SELECT `State`, `Name`, `LocationId`, '$role' as `Role`, '$employee' as `Emp_Id`, $tenentId as `Tenent_Id`  
		// 	FROM `Location` WHERE `State` in ('".implode("','", explode(",", $state))."') and `Tenent_Id` = '$tenentId' ";
		// }
		// else{
		// 	$insertEmpLocMapping .= "SELECT `State`, `Name`, `LocationId`, '$role' as `Role`, '$employee' as `Emp_Id`, $tenentId as `Tenent_Id` 
		// 	FROM `Location` WHERE `Site_Id` in ('".implode("','", explode(",", $siteId))."') and `Tenent_Id` = '$tenentId' ";
		// }
	...
	// $t=time();
	// $folderPath = "/var/www/DemoOneNetwork/files/".$t.'_';
	// $imageURL = "http://trinityapplab.com/DemoOneNetwork/files/";

	// if($inputTypeId == 18){
	// 	$writeFile = base64_to_jpeg($videoBase64,$folderPath.'Video');
	// 	$optionValue = $imageURL.substr($writeFile,30);
	// }
	// else if($inputTypeId == 19){
	// 	$writeFile = base64_to_jpeg($imageBase64,$folderPath.'Image');
	// 	$optionValue = $imageURL.substr($writeFile,30);
	// }
	...
	if($type == "new"){
		// if($categoryIcon != ""){
		// 	$writeFile = base64_to_jpeg($categoryIcon,$folderPath.'CategoryIcon');
		// 	$icons = $imageURL.substr($writeFile,30);
		// }
		// if($subcategory != "" && $subcategoryIcon != ""){
		// 	$writeFile = base64_to_jpeg($subcategoryIcon,$folderPath.'SubcategoryIcon');
		// 	$icons .= ','.$imageURL.substr($writeFile,30);
		// }
		// if($caption != "" && $captionIcon != ""){
		// 	$writeFile = base64_to_jpeg($captionIcon,$folderPath.'CaptionIcon');
		// 	$icons .= ','.$imageURL.substr($writeFile,30);
		// }
		...
		// $insertChecklist = "INSERT INTO `Menu`(`Cat`,`Sub`,`Caption`,`CheckpointId`,`Verifier`,`Approver`,`GeoFence`,`Icons`,`Active`,`Tenent_Id`) VALUES 
		// ('$category', '$subcategory','$caption','$checkpointId','$verifierId','$approverId','$geoFence','$icons',$active,$tenentId)";
	...
	$output = "";
	if($rowcount == 0){
		// $insertRole = "INSERT INTO `Role`(`Role`,`MenuId`,`Verifier_Role`,`Approver_Role`,`Tenent_Id`) VALUES ('$roleName', '$menuId', '$verifierRole', 
		// '$approvalRole', $tenentId)";
				...
				if(mysqli_query($conn,$insertMapping)){
					...
					if(mysqli_query($conn,$insertInTransHdr)){
						...
						if($lat != '' && $long !=''){
							...
							// if($inc == 0){
							// 	$insertLoc = "INSERT INTO `Location`(`GeoCoordinates`,`Tenent_Id`) VALUES ('$lat/$long',2)";
							// 	if(mysqli_query($conn,$insertLoc)){
							// 		$insertLocId = mysqli_insert_id($conn);
							// 		$updateLocIdMapping = "update Mapping set LocationId = '$insertLocId' where MappingId = $mappingId and 
							// 		Tenent_Id = $tenentId ";
							// 		mysqli_query($conn,$updateLocIdMapping);
							// 	}
							// }
							// else{
							// 	$updateLocIdMapping = "update Mapping set LocationId = '$insertLocId' where MappingId = $mappingId and 
	...
	$vendorSql = "SELECT e.VendorType FROM Employees e WHERE e.EmpId = '$loginEmpId'";
	$vendorQuery=mysqli_query($conn,$vendorSql);
	$vendorRow = mysqli_fetch_assoc($vendorQuery);
	$vendorType = $vendorRow["VendorType"];

	$svSql = "SELECT e.EmpId FROM Employees e WHERE e.Mobile = '$supervisorMobile'";
	...
	$locSql = "SELECT `LocationId` FROM `Location` WHERE `Name` = '$siteNamee' and `Site_Id` = '$siteId' ";
	...
	if($verifier_Role != null && $verifier_Role !=''){
		$sql2 = "SELECT el.Emp_Id FROM EmployeeLocationMapping el where el.LocationId = $locId and el.Role = '$verifier_Role' and el.Tenent_Id = $tenentId ";
...
// else if($insertType == "submitPtw_old" && $methodType === 'POST'){
// 	$loginEmpId = $jsonData->loginEmpId;
// 	$loginEmpName = $jsonData->loginEmpName;
// 	$browser = $jsonData->browser;
// 	$workStartDatetime = $jsonData->workStartDatetime;
...
// 	$verifierMobile = "";
// 	if($verifier_Role != null && $verifier_Role !=''){
// 		$sql2 = "SELECT el.Emp_Id, l.LocationId FROM Location l join EmployeeLocationMapping el on l.LocationId = el.LocationId where 
...
// 			$mappingId = $conn->insert_id;
// 			$status = true;
// 			$insertInTransHdr="INSERT INTO `TransactionHDR` (`ActivityId`, `Site_Id`, `Site_Name`, `Status`, `WorkStartDatetime`, `WorkEndDatetime`) VALUES ('$activityId', '$siteId', '$siteNamee', 'PTW_01', '$workStartDatetime', '$workEndDatetime')";
// 			if(mysqli_query($conn,$insertInTransHdr)){
// 				$lastTransHdrId = $conn->insert_id;
// 				$status = true;
