...
if($menuId == ""){
	$menuId = "283,285,287,313,314,315,316";
}
$output = array();
$wrappedList = [];
...
(case 
	when a.`MenuId` = 283 then concat(f.`Percentage`,' %') 
	when a.`MenuId` = 285 then concat(c.`Percentage`,' %') 
	when a.`MenuId` = 287 then concat(fb.`Percentage`,' %') 
	when a.`MenuId` = 313 then concat(vnt.`Percentage`,' %') 
	when a.`MenuId` = 314 then concat(fta.`Percentage`,' %') 
	when a.`MenuId` = 315 then concat(vnt2.`Percentage`,' %') 
	when a.`MenuId` = 316 then concat(ba.`Percentage`,' %') 
	else 'NA' end) as `Percentage`,    
(case 
	when a.`MenuId` = 283 then f.`Result` 
	when a.`MenuId` = 285 then c.`Result` 
	when a.`MenuId` = 287 then fb.`Result` 
	when a.`MenuId` = 313 then vnt.`Result` 
	when a.`MenuId` = 314 then fta.`Result` 
	when a.`MenuId` = 315 then vnt2.`Result` 
	when a.`MenuId` = 316 then ba.`Result` 
...
left join `FTA_Battery_Result` fta on `a`.`ActivityId` = `fta`.`ActivityId` 
left join `VNT_Training_Result2` vnt2 on `a`.`ActivityId` = `vnt2`.`ActivityId` 
left join `Basic_OnM` ba on `a`.`ActivityId` = `ba`.`ActivityId` 
...
while($row = mysqli_fetch_assoc($query)){
	...
	if($loopMenuId == 283 || $loopMenuId == 285 || $loopMenuId == 287 || $loopMenuId == 313 || $loopMenuId == 314 || $loopMenuId == 315 || $loopMenuId == 316){
