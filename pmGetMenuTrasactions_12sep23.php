...
UNION 
SELECT h.ActivityId FROM EmployeeLocationMapping el join Location l on el.LocationId = l.LocationId join TransactionHDR h on l.Site_Id = h.Site_Id join Activity a on h.ActivityId = a.ActivityId and a.MenuId in ($menuId) and a.Event = 'Submit' WHERE el.`Emp_Id` in ('$loginEmpId') 
) t";
...
'' as `Percentage`,    
'' as `Result`,
sp.`ok_count`, sp.`not_ok_count`, m.`Verifier` as verifierEmpId, m.`Approver` as approverEmpId   
...
left join `Site_PM_OK_NOTOK` sp on `h`.`ActivityId` = `sp`.`ActivityId`
