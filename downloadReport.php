...
// else {
	...
	// Incident Report
		...
		$sql = "SELECT (@sr := @sr+1) as `Sr. No.`, `ActivityId`, `Circle`, `Site Name`, `Site Id`, `Incident category`, `Material Damaged`, `Incident Date`, `Incident Time`, `Incident description`, `Location (Lat Long)`, `Entered By`, `Approved status By L1`, `Approved status By L2` FROM (select @sr:=0) as sr, `Incident_Management_Report` where 1=1 ".$filterSql;
		if($fromDate != "")
	...
	// PM Report
		...
		if($loginEmpRole != 'Admin' && $loginEmpRole != 'SpaceWorld' && $loginEmpRole != "Management" && $loginEmpRole != "Corporate OnM lead"){
			...
			$el = implode("','", $empSiteList);
			$filterSql .= "and h.Site_Id in ('".$el."') ";
		}

		if($fromDate != "")
			$filterSql .= "and DATE_FORMAT(a.MobileDateTime,'%Y-%m-%d') >= '$fromDate' ";
		if($toDate != "")
			$filterSql .= "and DATE_FORMAT(a.MobileDateTime,'%Y-%m-%d') <= '$toDate' ";

		$sql = "SELECT max(t.State) as Circle, max(t.Site_Name) as Site_Name, max(t.Site_Id) as Site_Id, max(t.`Site Type`) as `Site Type`, max(t.`PM Done Date`) as `PM Done Date`, max(t.`Airtel Site Id`) as `Airtel Site Id`, max(t.`Airtel Load`) as `Airtel Load`, max(t.`MTNL/BSNL Site Id`) as `MTNL/BSNL Site Id`, max(t.`MTNL/BSNL Load`) as `MTNL/BSNL Load`, max(t.`VIL Site Id`) as `VIL Site Id`, max(t.`VIL Load`) as `VIL Load`, max(t.`RJIO Site Id`) as `RJIO Site Id`, max(t.`RJIO Load`) as `RJIO Load`, max(t.`No. of FE`) as `No. of FE`, max(t.`Serial No. OF FE 1`) as `Serial No. OF FE 1`, max(t.`Refilling date of FE 1`) as `Refilling date of FE 1`, max(t.`Expiry date of FE 1`) as `Expiry date of FE 1`, max(t.`Serial No. OF FE 2`) as `Serial No. OF FE 2`, max(t.`Refilling date of FE 2`) as `Refilling date of FE 2`, max(t.`Expiry date of FE 2`) as `Expiry date of FE 2`, max(t.`Serial No. OF FE 3`) as `Serial No. OF FE 3`, max(t.`Refilling date of FE 3`) as `Refilling date of FE 3`, max(t.`Expiry date of FE 3`) as `Expiry date of FE 3`, max(t.`Serial No. OF FE 4`) as `Serial No. OF FE 4`, max(t.`Refilling date of FE 4`) as `Refilling date of FE 4`, max(t.`Expiry date of FE 4`) as `Expiry date of FE 4`, max(t.`Serial No. OF FE 5`) as `Serial No. OF FE 5`, max(t.`Refilling date of FE 5`) as `Refilling date of FE 5`, max(t.`Expiry date of FE 5`) as `Expiry date of FE 5`, max(t.`Pole Type`) as `Pole Type`, max(t.`No. of Pole`) as `No. of Pole`, max(t.`Pole Height`) as `Pole Height`, max(t.`Airtel RRH`) as `Airtel RRH`, max(t.`Airtel MW`) as `Airtel MW`, max(t.`Airtel GSM`) as `Airtel GSM`, max(t.`MTNL/BSNL RRH`) as `MTNL/BSNL RRH`, max(t.`MTNL/BSNL MW`) as `MTNL/BSNL MW`, max(t.`MTNL/BSNL GSM`) as `MTNL/BSNL GSM`, max(t.`VIL RRH`) as `VIL RRH`, max(t.`VIL MW`) as `VIL MW`, max(t.`VIL GSM`) as `VIL GSM`, max(t.`RJIO RRH`) as `RJIO RRH`, max(t.`RJIO MW`) as `RJIO MW`, max(t.`RJIO GSM`) as `RJIO GSM`, max(t.`SMPS Make`) as `SMPS Make`, (max(t.`No. of RM`) + max(t.`No. of faulty`)) as `No. of RM`, max(t.`No. of faulty`) as `No. of faulty`, max(t.`BB Make & Model`) as `BB Make & Model`, max(t.`No. of BB`) as `No. of BB`, max(t.`SOC &SOH status`) as `SOC &SOH status`, max(t.`Capacity in AH 1`) as `Capacity in AH 1`, max(t.`Capacity in AH 2`) as `Capacity in AH 2`, max(t.`Capacity in AH 3`) as `Capacity in AH 3`, max(t.Name) as `PM done By`, max(t.`PM approved By:L1`) as `PM approved By:L1` from 
			(SELECT a.ActivityId, l.State, l.Site_CAT, l.Airport_Metro, l.RFI_Date, l.High_Revenue_Site, l.ISQ, l.Retail_IBS, l.Is_Active as Is_Site_Active, a.EmpId, e.Name, h.Site_Id, h.Site_Name, 
			(case when d.ChkId = 5058 then d.Value end) as `Site Type`,
			a.MobileDateTime as `PM Done Date`,
			date_format(a.MobileDateTime,'%b-%Y') as PM_done_period,
			date_format(l.RFI_Date,'%b-%Y') as RFI_Date_period,
			(case when d.ChkId = 4716 then d.Value end) as `Airtel Site Id`,
			(case when d.ChkId = 5074 then d.Value end) as `Airtel Load`,
			(case when d.ChkId = 5084 then d.Value end) as `MTNL/BSNL Site Id`,
			(case when d.ChkId = 5080 then d.Value end) as `MTNL/BSNL Load`,
			(case when d.ChkId = 5089 then d.Value end) as `VIL Site Id`,
			(case when d.ChkId = 5085 then d.Value end) as `VIL Load`,
			(case when d.ChkId = 5094 then d.Value end) as `RJIO Site Id`,
			(case when d.ChkId = 5090 then d.Value end) as `RJIO Load`,
			(case when d.ChkId = 5059 then d.Value end) as `No. of FE`,
			(case when d.ChkId = 5060 then d.Value end) as `Serial No. OF FE 1`,
			(case when d.ChkId = 5061 then d.Value end) as `Refilling date of FE 1`,
			(case when d.ChkId = 5064 then d.Value end) as `Expiry date of FE 1`,
			(case when d.ChkId = 5097 then d.Value end) as `Serial No. OF FE 2`,
			(case when d.ChkId = 5096 then d.Value end) as `Refilling date of FE 2`,
			(case when d.ChkId = 5095 then d.Value end) as `Expiry date of FE 2`,
			(case when d.ChkId = 5100 then d.Value end) as `Serial No. OF FE 3`,
			(case when d.ChkId = 5099 then d.Value end) as `Refilling date of FE 3`,
			(case when d.ChkId = 5098 then d.Value end) as `Expiry date of FE 3`,
			(case when d.ChkId = 5103 then d.Value end) as `Serial No. OF FE 4`,
			(case when d.ChkId = 5102 then d.Value end) as `Refilling date of FE 4`,
			(case when d.ChkId = 5101 then d.Value end) as `Expiry date of FE 4`,
			(case when d.ChkId = 5106 then d.Value end) as `Serial No. OF FE 5`,
			(case when d.ChkId = 5105 then d.Value end) as `Refilling date of FE 5`,
			(case when d.ChkId = 5104 then d.Value end) as `Expiry date of FE 5`,
			(case when d.ChkId = 4670 then d.Value end) as `Pole Type`,
			(case when d.ChkId = 4784 then d.Value end) as `No. of Pole`,
			(case when d.ChkId = 4671 then d.Value end) as `Pole Height`,
			(case when d.ChkId = 5131 then d.Value end) as `Airtel RRH`,
			(case when d.ChkId = 4787 then d.Value end) as `Airtel MW`,
			(case when d.ChkId = 4788 then d.Value end) as `Airtel GSM`,
			(case when d.ChkId = 5132 then d.Value end) as `MTNL/BSNL RRH`,
			(case when d.ChkId = 5107 then d.Value end) as `MTNL/BSNL MW`,
			(case when d.ChkId = 5108 then d.Value end) as `MTNL/BSNL GSM`,
			(case when d.ChkId = 5133 then d.Value end) as `VIL RRH`,
			(case when d.ChkId = 5111 then d.Value end) as `VIL MW`,
			(case when d.ChkId = 5110 then d.Value end) as `VIL GSM`,
			(case when d.ChkId = 5134 then d.Value end) as `RJIO RRH`,
			(case when d.ChkId = 5113 then d.Value end) as `RJIO MW`,
			(case when d.ChkId = 5114 then d.Value end) as `RJIO GSM`,
			(case when d.ChkId = 4703 then d.Value end) as `SMPS Make`,
			(case when d.ChkId = 4777 then d.Value end) as `No. of RM`,
			(case when d.ChkId = 4704 then d.Value end) as `No. of faulty`,
			(case when d.ChkId = 4710 then d.Value end) as `BB Make & Model`,
			(case when d.ChkId = 4779 then d.Value end) as `No. of BB`,
			(case when d.ChkId = 4707 then d.Value end) as `SOC &SOH status`,
			(case when d.ChkId = 4778 then d.Value end) as `Capacity in AH 1`,
			(case when d.ChkId = 5116 then d.Value end) as `Capacity in AH 2`,
			(case when d.ChkId = 5117 then d.Value end) as `Capacity in AH 3`,
			m.Verifier as `Verifier_Emp_Id`,
			(case when h.VerifierActivityId is null then 'Pending' else e1.Name end) as `PM approved By:L1`,
			m.Approver as `Approver_Emp_Id`,
			(case when h.ApproverActivityId is null then 'Pending' else e2.Name end) as `PM approved By:L2`
			FROM Activity a 
			join TransactionHDR h on a.ActivityId = h.ActivityId 
			join Location l on h.Site_Id = l.Site_Id and h.Site_Name = l.Name
			left join TransactionDTL d on h.ActivityId = d.ActivityId 
			left join Employees_Reference e on a.EmpId = e.EmpId and a.RefId = e.RefId
			left join Mapping m on a.ActivityId = m.ActivityId
			left join Activity a1 on a1.ActivityId = h.VerifierActivityId 
			left join Activity a2 on a2.ActivityId = h.ApproverActivityId 
			left join Employees_Reference e1 on e1.EmpId = a1.EmpId and e1.RefId = a1.RefId 
			left join Employees_Reference e2 on e2.EmpId = a2.EmpId and e2.RefId = a2.RefId 
			where a.MenuId = 274 and a.Event = 'Submit' ".$filterSql.") t GROUP by t.ActivityId";
	...
	// Export Supervisor
		...
		$sql = "SELECT e1.`Name`, e1.`Mobile`, e1.`Whatsapp_Number` as `Whatsapp`, e1.`AadharCard_Number` as `Aadhar card`, (case when e2.`RoleId` = 53 then e2.`Name` else e3.`Name` end) as `Vendor Name`, e1.`Active` FROM `Employees` e1 join `Employees` e2 on e1.`RMId` = e2.`EmpId` left join `Employees` e3 on e2.`RMId` = e3.`EmpId` where e1.`RoleId` = 58 ".$filterSql." ";
	...
	// Supervisor Training
		...
		$sql = "SELECT e.Name as `Supervisor Name`, e1.Name as `Partner Name`, st.TrainingType as `Type Of Training`, st.CompanyName as `Company Name (Training Provide)`, st.TrainingDate as `Date of Registration`, st.TrainingIdNo as `Training ID`, st.TrainingExDate as `Expire Date`, st.TrainingGivenBy as `Training Given By`, st.ModeOfTraining as `Mode of Training`, (case when st.Status = 1 then 'Approve' when st.Status = 2 then 'Reject' else 'Pending' end) as `Status`, st.RejectReason, st.TrainingPic as `External Certification / Training Snaps` FROM SupervisorTraining st join Employees e on st.EmpId = e.EmpId left join Employees e1 on e.RMId = e1.EmpId where st.IsDeleted = 0";
		...
		header('Content-Type: text/csv; charset=utf-8');
		header('Content-Disposition: attachment; filename=SupervisorTraining.csv');
		$output = fopen('php://output', 'w');
		fputcsv($output, $columnName);
	...
	// Supervisor PTW
		...
		$sql = "SELECT a.PartnerName as `Partner Name`, a.Name as `Supervisor Name`, a.SupervisorMobile as `Supervisor Mobile`, a.Height, a.Electrical, a.MaterialHandling, a.OfcRouteWork, a.ConfinedSpaceWork, a.HotWork, a.SiteAccess, d.TotalDone as `Number of PTW Site & Team assessments done`, (a.TotalAssign - d.TotalDone) as `Number of PTW activity not done`  FROM SupervisorPTWAssign a join SupervisorPTWDone d on a.Approver = d.Approver";
		$result = mysqli_query($conn,$sql);
		$row=mysqli_fetch_assoc($result);
		$columnName = array();
		foreach ($row as $key => $value) {
		...
		header('Content-Type: text/csv; charset=utf-8');
		header('Content-Disposition: attachment; filename=SupervisorPTW.csv');
		$output = fopen('php://output', 'w');
		fputcsv($output, $columnName);
