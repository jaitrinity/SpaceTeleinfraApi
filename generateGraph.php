...
if($graphType == 1){
	...
	if($loginEmpRole != "Admin" && $loginEmpRole != "SpaceWorld" && $loginEmpRole != "Management" && $loginEmpRole != "CB" && $loginEmpRole != "Corporate OnM lead"){
		...
		$empMapSql = "SELECT l.Site_Id  FROM EmployeeLocationMapping el join Location l on el.LocationId = l.LocationId and l.Is_NBS_Site = 0 and l.Tenent_Id = 2 and l.Is_Active = 1 WHERE `Emp_Id` = '$loginEmpId'";
		$empMapQuery = mysqli_query($conn,$empMapSql);
		$empMapRowCount = mysqli_num_rows($empMapQuery);
		$empMapSiteList = array();
		if($empMapRowCount !=0){
			while ($empMapRow = mysqli_fetch_assoc($empMapQuery)){
				array_push($empMapSiteList, $empMapRow["Site_Id"]);
			}
		}
		$empMapSite = implode("','", $empMapSiteList);
		$filterSql .= "and `Site_Id` in ('".$empMapSite."')";
	}
	$sql = "";
	if($incidentCategory == ""){
...
// Fiber Cut Incident and MTTR of Fiber Cut
	...
	if($loginEmpRole != "Admin" && $loginEmpRole != "SpaceWorld" && $loginEmpRole != "Management" && $loginEmpRole != "CB" && $loginEmpRole != "Corporate OnM lead"){
		...
		$empMapSite = implode("','", $empMapSiteList);
		$filterSql .= "and im.`Site Id` in ('".$empMapSite."')";
	}
...
// Metro and Airport sites PM
	...
	if($loginEmpRole != "Admin" && $loginEmpRole != "SpaceWorld" && $loginEmpRole != "Management" && $loginEmpRole != "CB" && $loginEmpRole != "Corporate OnM lead"){
		...
		$empMapSite = implode("','", $empMapSiteList);
		$filterSql .= "and p.`Site_Id` in ('".$empMapSite."')";
	}
...
// PM punchpoint
	...
	if($loginEmpRole != "Admin" && $loginEmpRole != "SpaceWorld" && $loginEmpRole != "Management" && $loginEmpRole != "CB" && $loginEmpRole != "Corporate OnM lead"){
		...
		$empMapSite = implode("','", $empMapSiteList);
		$filterSql .= "and p.`Site Id` in ('".$empMapSite."')";
	}
...
// Metro and Airport sites PM 2
	...
	$sql = "SELECT t.Site_Type, t.Site_count, t.PM_done, round((t.PM_done/t.Site_count)*100,0) as 'Done_Percentage' from (SELECT t.Site_Type, (select count(*) from Location where (RFI_date is null or RFI_date < '".$quarterStartDate."') and Is_NBS_Site = 0 and Airport_Metro = 'Airport Site' and Is_Active=1) as Site_count, count(t.Site_Type) as PM_done from (SELECT DISTINCT p.Site_Id, p.Airport_Metro as Site_Type FROM PM_Graph p where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') ".$filterSql." and p.Airport_Metro = 'Airport Site' and p.Is_Site_Active =1) t GROUP by t.Site_Type
		...
		UNION
		SELECT t.Site_Type, (select count(*) from Location where (RFI_date is null or RFI_date < '".$quarterStartDate."') and Is_NBS_Site = 0 and Airport_Metro = 'GMRC' and Is_Active=1) as Site_count, count(t.Site_Type) as PM_done from (SELECT DISTINCT p.Site_Id, p.Airport_Metro as Site_Type FROM PM_Graph p where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') ".$filterSql." and p.Airport_Metro = 'GMRC' and p.Is_Site_Active =1) t GROUP by t.Site_Type
		UNION
