...
// Site Type wise PM
	...
	$gtSql = "SELECT gt1.State, gt1.Site_count, gt1.PM_done, round((gt1.PM_done/gt1.Site_count)*100,0) as 'Done_Percentage' from (select 'Total ' as State, sum(gt.Site_count) as Site_count, sum(gt.PM_done) as PM_done from (SELECT t3.State, t3.Site_count, t3.PM_done, round((t3.PM_done/t3.Site_count)*100,0) as 'Done_Percentage' from (select s.State, s.Site_count, (case when t2.PM_done is null then 0 else t2.PM_done end) as PM_done from (SELECT l.State, count(l.Site_Id) as Site_count FROM Location l where l.Site_Id is not null and l.Site_Id != '' ".$siteTypeSql." and l.Tenent_Id = 2 and l.Is_Active = 1 GROUP BY l.State".$gropupBySql." ) s left join (select t1.State, sum(t1.Single_count) as PM_done from (select t.State, t.Site_Id, t.State_site_id, count(t.State_site_id) as Done_count, (case when count(t.State_site_id) > 1 then 1 else count(t.State_site_id) end) as Single_count from (SELECT State, Site_Id, concat(State,' - ',Site_Id) as State_site_id FROM PM_Graph 
		where PM_done_period in ('".$quarter."') and (RFI_Date_period is null or RFI_Date_period not in ('".$quarter."')) and (RFI_date is null or RFI_date < '".$quarterStartDate."') and Is_Site_Active = 1 ".$filterSql.") t GROUP by t.State_site_id) t1 GROUP by t1.State) t2 on s.State = t2.State ) t3) gt) gt1 ";
...
// Metro and Airport sites PM
	...
	if($metroSiteType == "High_R_Site"){
		$sql1 = "SELECT t.Site_Type, count(t.Site_Type) as Site_Count from (SELECT concat(p.Site_Id,' - ',p.Site_Name) as Site_Id, 'High Revenue Site' as Site_Type FROM PM_Graph p 
		where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') and p.Is_Site_Active = 1 and p.High_Revenue_Site = 1 ".$filterSql.") t GROUP by t.Site_Type";
	}
	else if($metroSiteType == "ISQ"){
		$sql1 = "SELECT t.Site_Type, count(t.Site_Type) as Site_Count from (SELECT DISTINCT concat(p.Site_Id,' - ',p.Site_Name) as Site_Id, 'ISQ' as Site_Type FROM PM_Graph p 
		where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') and p.Is_Site_Active = 1 and p.ISQ = 1 ".$filterSql.") t GROUP by t.Site_Type";
	}
	else if($metroSiteType == "Retail_IBS"){
		$sql1 = "SELECT t.Site_Type, count(t.Site_Type) as Site_Count from (SELECT DISTINCT concat(p.Site_Id,' - ',p.Site_Name) as Site_Id, 'Retail IBS' as Site_Type FROM PM_Graph p 
		where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') and p.Is_Site_Active = 1 and p.Retail_IBS = 1 ".$filterSql.") t GROUP by t.Site_Type";
	}
	else{
		$sql1 = "SELECT t.Site_Type, count(t.Site_Type) as Site_Count from (SELECT DISTINCT concat(p.Site_Id,' - ',p.Site_Name) as Site_Id, p.Airport_Metro as Site_Type FROM PM_Graph p 
		where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') and p.Is_Site_Active = 1 and p.Airport_Metro = '$metroSiteType' ".$filterSql.") t GROUP by t.Site_Type";
	}
...
// Metro and Airport sites PM 2
	...
	$sql = "SELECT t.Site_Type, t.Site_count, t.PM_done, round((t.PM_done/t.Site_count)*100,0) as 'Done_Percentage' from (SELECT t.Site_Type, (select count(*) from Location where (RFI_date is null or RFI_date < '".$quarterStartDate."') and Airport_Metro = 'Airport Site' and Is_Active=1) as Site_count, count(t.Site_Type) as PM_done from (SELECT DISTINCT concat(p.Site_Id,' - ',p.Site_Name) as Site_Id, p.Airport_Metro as Site_Type FROM PM_Graph p where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') ".$filterSql." and p.Airport_Metro = 'Airport Site' and p.Is_Site_Active =1) t GROUP by t.Site_Type
		UNION
		SELECT t.Site_Type, (select count(*) from Location where (RFI_date is null or RFI_date < '".$quarterStartDate."') and Airport_Metro = 'CMRL' and Is_Active=1) as Site_count, count(t.Site_Type) as PM_done from (SELECT DISTINCT concat(p.Site_Id,' - ',p.Site_Name) as Site_Id, p.Airport_Metro as Site_Type FROM PM_Graph p where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') ".$filterSql." and p.Airport_Metro = 'CMRL' and p.Is_Site_Active =1) t GROUP by t.Site_Type
		UNION
		SELECT t.Site_Type, (select count(*) from Location where (RFI_date is null or RFI_date < '".$quarterStartDate."') and Airport_Metro = 'DMRC' and Is_Active=1) as Site_count, count(t.Site_Type) as PM_done from (SELECT DISTINCT concat(p.Site_Id,' - ',p.Site_Name) as Site_Id, p.Airport_Metro as Site_Type FROM PM_Graph p where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') ".$filterSql." and p.Airport_Metro = 'DMRC' and p.Is_Site_Active =1) t GROUP by t.Site_Type
		UNION
		SELECT t.Site_Type, (select count(*) from Location where (RFI_date is null or RFI_date < '".$quarterStartDate."') and Airport_Metro = 'JMRC' and Is_Active=1) as Site_count, count(t.Site_Type) as PM_done from (SELECT DISTINCT concat(p.Site_Id,' - ',p.Site_Name) as Site_Id, p.Airport_Metro as Site_Type FROM PM_Graph p where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') ".$filterSql." and p.Airport_Metro = 'JMRC' and p.Is_Site_Active =1) t GROUP by t.Site_Type
		UNION
		...
		UNION
		SELECT t.Site_Type, (select count(*) from Location where (RFI_date is null or RFI_date < '".$quarterStartDate."') and Retail_IBS = 1 and Is_Active=1) Site_count, count(t.Site_Type) as PM_done from (SELECT DISTINCT concat(p.Site_Id,' - ',p.Site_Name) as Site_Id, 'Retail IBS' as Site_Type FROM PM_Graph p where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') ".$filterSql." and p.Retail_IBS = 1 and p.Is_Site_Active =1) t GROUP by t.Site_Type
		UNION
		SELECT t.Site_Type, (select count(*) from Location where (RFI_date is null or RFI_date < '".$quarterStartDate."') and ISQ = 1 and Is_Active=1) Site_count, count(t.Site_Type) as PM_done from (SELECT DISTINCT concat(p.Site_Id,' - ',p.Site_Name) as Site_Id, 'ISQ' as Site_Type FROM PM_Graph p where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') ".$filterSql." and p.ISQ = 1 and p.Is_Site_Active =1) t GROUP by t.Site_Type
		UNION
		SELECT t.Site_Type, (select count(*) from Location where (RFI_date is null or RFI_date < '".$quarterStartDate."') and High_Revenue_Site = 1 and Is_Active=1) Site_count, count(t.Site_Type) as PM_done from (SELECT DISTINCT concat(p.Site_Id,' - ',p.Site_Name) as Site_Id, 'High_R_Site' as Site_Type FROM PM_Graph p where p.PM_done_period in ('".$quarter."') and (p.RFI_Date_period is null or p.RFI_Date_period not in ('".$quarter."')) and (p.RFI_date is null or p.RFI_date < '".$quarterStartDate."') ".$filterSql." and p.High_Revenue_Site = 1 and p.Is_Site_Active =1) t GROUP by t.Site_Type) t";
